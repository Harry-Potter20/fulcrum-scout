...
script. This is a genuine stdlib HTTP server — no framework dependency, minimal attack surface, easy to reason about.
    ...
    GET /sync?key=<FULCRUM_SYNC_KEY>   -> runs jobs/sync_db.run(), 200 {"status":"ok","upserted":N}
...
/sync request is rejected (503), so this can be deployed with the endpoint present but inert until configured.
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if not SYNC_KEY:
            return self._json(503, {"error": "FULCRUM_SYNC_KEY not configured"})
